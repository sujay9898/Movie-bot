...
2. Paste the Web App URL into the `WEB_APP_URL` variable:
   ```javascript
   let WEB_APP_URL = 'https://script.google.com/macros/s/YOUR_SCRIPT_ID/exec';
   ```
3. Save the file
4. Select `setWebhook` from the function dropdown (top menu)
5. Click **Run**
6. Check the Execution log - should show `"ok":true`
...
3. Update the admin list in the script:
   ```javascript
   const adminIds = ['YOUR_TELEGRAM_USER_ID'];
   ```
...
- Forward movie files to add them to database
- Inline buttons for easy downloading
